...
print.PPA_NPA <- function(data1, data0) {
  ...
  model1 <- glmer(Response ~ (1 | INFANTID) + (1 | SITE),
                  data = data1, family = binomial)
  model0 <- glmer(Response ~ (1 | INFANTID) + (1 | SITE),
...
glm_model_TCB <- lmer(tcb ~ (1 | INFANTID) + (1 | SITE),
                      data = df_tcb_long)
glm_model_JAUNDICE <- glmer(jaundice ~ (1 | INFANTID) + (1 | SITE),
...
df_tcb_long_threshold_method<-df_tcb_long%>%
  ...
  pivot_longer(
    ...
    strata_id = paste(SITE, INFANTID, sep = "_")
  )
