...
##########glmm########
#random intercept for MOMID and TYPE_VISIT
model1<-glmer(Response ~ (1|MOMID)+(1|TYPE_VISIT), data = temp_df1, family = binomial)
summary(model1)

model0<-glmer(Response ~ (1|MOMID)+(1|TYPE_VISIT), data = temp_df0, family = binomial)
summary(model0)

#random intercept for MOMID, TYPE_VISIT and SITE
model1_site<-glmer(Response ~ (1|MOMID)+(1|SITE)+(1|TYPE_VISIT), data = temp_df1, family = binomial)
summary(model1_site)

model0_site<-glmer(Response ~ (1|MOMID)+(1|SITE)+(1|TYPE_VISIT), data = temp_df0, family = binomial)
summary(model0_site)
