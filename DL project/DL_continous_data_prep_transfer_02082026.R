...
data.temporal <- data.temporal %>%
  mutate(
    Region = case_when(
      SITE %in% c("Pakistan","India-SAS","India-CMC") ~ "Asia",
      SITE %in% c("Zambia","Ghana","Kenya") ~ "Africa",
      TRUE ~ NA_character_
    )
  )

africa_ids <- data.temporal %>%
  filter(Region == "Africa") %>%
  distinct(SITE, INFANTID, MOMID, PREGID) %>%
  mutate(u = runif(n())) %>%
  mutate(AFR_SPLIT = ifelse(u <= threshold, "AFR_TRAIN", "AFR_TEST")) %>%
  select(-u)


data.temporal <- data.temporal %>%
  left_join(africa_ids, by = c("SITE", "INFANTID", "MOMID", "PREGID")) %>%
  mutate(
    IMP_GROUP = case_when(
      Region == "Asia" ~ "ASIA_ALL",
      Region == "Africa" & AFR_SPLIT == "AFR_TRAIN" ~ "AFR_TRAIN",
      Region == "Africa" & AFR_SPLIT == "AFR_TEST" ~ "AFR_TEST",
      TRUE ~ NA_character_
    )) %>% select(-AFR_SPLIT)

data.temporal.mask <- data.temporal %>%
  mutate(across(-c(SITE, MOMID, INFANTID, PREGID, TYPE_VISIT, Region), ~ ifelse(is.na(.), 1, 0)))

data.temporal.mean <- data.temporal %>%
  filter(IMP_GROUP %in% c("ASIA_ALL", "AFR_TRAIN")) %>%
  group_by(IMP_GROUP) %>%
  summarise(across(-c(SITE, MOMID, INFANTID ,PREGID, TYPE_VISIT, PREG_START_DATE, M04_ANC_OBSSTDAT, M08_LBSTDAT, M01_US_OHOSTDAT, Region), ~ round(mean(.x, na.rm = TRUE), 2))) %>%
  mutate(across(c( 
    # Iron supplement
    M04_IRON_CMOCCUR,
    M04_IRON_ORAL_CMOCCUR,
...
data.temporal.GA <- data.temporal %>%
  filter(IMP_GROUP %in% c("ASIA_ALL", "AFR_TRAIN")) %>%
  group_by(IMP_GROUP, TYPE_VISIT) %>%
  summarise(
    across(c(M04_GA_days, M08_GA_days, M01_GA_days), ~ round(mean(.x, na.rm = TRUE), 2)),
    .groups = "drop"
  )
...
data.temporal.impute.pre <- data.temporal %>%
  ...
  mutate(across(-c(TYPE_VISIT, PREG_START_DATE, M04_ANC_OBSSTDAT, M08_LBSTDAT, M01_US_OHOSTDAT, M04_GA_days, M08_GA_days, M01_GA_days, Region, IMP_GROUP), 
...
data.temporal.impute.pre <- data.temporal.impute.pre %>%
  mutate(
    IMP_GROUP_FOR_GA = ifelse(IMP_GROUP == "AFR_TEST", "AFR_TRAIN", IMP_GROUP)
  ) %>%
  left_join(
    data.temporal.GA,
    by = c("IMP_GROUP_FOR_GA" = "IMP_GROUP", "TYPE_VISIT"),
    suffix = c("", "_mean")
  ) %>%
  mutate(
  ...
  select(-M04_GA_days_mean, -M08_GA_days_mean, -M01_GA_days_mean, -IMP_GROUP_FOR_GA)


data.temporal.impute <- data.temporal.impute.pre %>%
  mutate(
    IMP_GROUP_FOR_MEAN = ifelse(IMP_GROUP == "AFR_TEST", "AFR_TRAIN", IMP_GROUP)
  ) %>%
  left_join(
    data.temporal.mean,
    by = c("IMP_GROUP_FOR_MEAN" = "IMP_GROUP"),
    suffix = c("", "_popmean")
  ) %>% {
    tmp <- .
    
    # variables available in the mean table (excluding the key)
    mean_vars <- setdiff(names(data.temporal.mean), "IMP_GROUP")
    
    # only impute variables that actually exist in tmp (avoid IDs/dates/etc.)
    impute_vars <- intersect(mean_vars, names(tmp))
    
    # For each variable v, fill NA in v using v_popmean
    for (v in impute_vars) {
      pop_col <- paste0(v, "_popmean")
      if (pop_col %in% names(tmp)) {
        tmp[[v]] <- dplyr::coalesce(tmp[[v]], tmp[[pop_col]])
      }
    }
    
    tmp %>%
      dplyr::select(-dplyr::ends_with("_popmean"), -IMP_GROUP_FOR_MEAN)
  }
...
# get one IMP_GROUP per infant (from temporal)
inf_group <- data.temporal %>%
  distinct(SITE, INFANTID, MOMID, PREGID, IMP_GROUP)

data.static.impute.pre <- data.static %>%
  left_join(inf_group, by = c("SITE","INFANTID","MOMID","PREGID"))

static_mean <- data.static.impute.pre %>%
  filter(IMP_GROUP %in% c("AFR_TRAIN", "ASIA_ALL")) %>%
  group_by(IMP_GROUP) %>%
  summarise(
    across(
...
data.static.impute <- data.static.impute.pre %>%
  mutate(IMP_GROUP_FOR_MEAN = ifelse(IMP_GROUP == "AFR_TEST", "AFR_TRAIN", IMP_GROUP)) %>%
  left_join(static_mean, by = c("IMP_GROUP_FOR_MEAN" = "IMP_GROUP"), suffix = c("", "_mean")) %>%
  mutate(
    # binary/categorical: fill NA with 0 (as you do)
    across(
      -c(SITE, INFANTID, MOMID, PREGID, IMP_GROUP, IMP_GROUP_FOR_MEAN,
         INF_ASPH, STILLBIRTH_SIGNS_LIFE, SVN, PRETERMBIRTH_LT37, 
    ...
    # continuous: fill NA with group-specific mean
    MAT_AGE = coalesce(MAT_AGE, MAT_AGE_mean),
    MUAC_ENROLL = coalesce(MUAC_ENROLL, MUAC_ENROLL_mean),
    GESTAGEBIRTH_ANY = coalesce(GESTAGEBIRTH_ANY, GESTAGEBIRTH_ANY_mean),
    BWEIGHT_ANY = coalesce(BWEIGHT_ANY, BWEIGHT_ANY_mean),
    CROWDING_IND = coalesce(CROWDING_IND, CROWDING_IND_mean),
    BMI_ENROLL = coalesce(BMI_ENROLL, BMI_ENROLL_mean)
  ) %>%
  select(-ends_with("_mean"), -IMP_GROUP_FOR_MEAN)


data.static.merge <- data.static.impute %>%
...
write.csv(data.dl.continuous,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_continuous_transfer.csv")
write.csv(data.static,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_static_transfer.csv")
...
data.temporal.impute.wide  <- data.temporal.impute  %>% 
  ...
  pivot_wider(
    ...
    values_from = -c(SITE,INFANTID,MOMID,PREGID,TYPE_VISIT,IMP_GROUP)  # Columns to widen
  )

data.static.merge.wide <- data.static.impute %>% select(-c(SITE ,INFANTID, MOMID, PREGID))
...
write.csv(data.temporal.wide,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_temporal_wide_transfer.csv")
write.csv(data.dl.continuous.wide,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_continuous_wide_transfer.csv")

write.csv(data.dl.continuous.delta,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_continuous_delta_transfer.csv")
write.csv(data.dl.continuous.mask,"D:/Users/yipeng_wei/Documents/dl data/2025-10-31/df_dl_continuous_mask_transfer.csv")
