...
lr.function <- function(data1,data2,outcome) {
  ...
  if (outcome == "STILLBIRTH_SIGNS_LIFE") {
    ...
    features_lr <- c("STILLBIRTH_IND","PRETERM_IND","CESARIAN_IND",
                     ...
                     "HTN","DIAB_OVERT_ANY",
                     "GES_HTN","DIAB_GEST_ANY",
                     "INF_SEX","GESTAGEBIRTH_ANY","BWEIGHT_ANY","INF_PSBI_ANY","INF_HYPERBILI_NICE")
...
data.dl.continuous.wide.Africa.lr <- data.dl.continuous.wide.Africa.lr %>%
  arrange(X)

data.dl.continuous.Africa.lr$svn.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"SVN"),each=5)
data.dl.continuous.Africa.lr$stillbirth.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"STILLBIRTH_SIGNS_LIFE"),each=5)
data.dl.continuous.Africa.lr$lbw.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"LBW2500_ANY"),each=5)
data.dl.continuous.Africa.lr$preterm.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"PRETERMBIRTH_LT37"),each=5)
data.dl.continuous.Africa.lr$neo_dth.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"NEO_DTH"),each=5)
data.dl.continuous.Africa.lr$nearmiss.prob<-rep(lr.function(data.dl.continuous.wide.Africa.lr,data.dl.continuous.wide.Africa.lr,"NEARMISS"),each=5)
